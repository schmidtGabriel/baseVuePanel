...
<script>
import CivilStateForm from "src/components/UIComponents/Forms/CivilStateForm.vue";
...
export default {
  ...
  methods: {
    ...mapActions(["updateCivilState", "getCivilState"]),
    ...mapGetters({
  ...
  created() {
    ...
    this.$on("save", () => {
        this.updateCivilState(this.data)
            .then(res => {
               this.$notify({
                group: "foo",
                title: "Sucesso",
