...
<script>
  ...
  const headers = ["Nome","Data Cadastrado"];
  const tableColumns = [ 
    "name", 
    {
  ...
  export default {
    ...
    computed: {
      ...mapState({
        civilstateList: ({civilstate}) => civilstate.list
      }),
      ...mapGetters({
    ...
    methods: {
      ...mapActions(["getCivilStateList", "deleteCivilState"]),
        ...
        buttons: [
          {
            ...
            handler: () => { 
                ...
                const index = this.civilstateList.indexOf(item, 0);
                  if (index > -1) {
                      this.civilstateList.splice(index, 1);
                  }
