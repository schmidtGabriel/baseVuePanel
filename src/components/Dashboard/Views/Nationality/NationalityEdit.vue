...
<script>
import NationalityForm from "src/components/UIComponents/Forms/NationalityForm.vue";
...
export default {
  ...
  methods: {
    ...mapActions(["updateNationality", "getNationality"]),
    ...mapGetters({
  ...
  created() {
    ...
    this.$on("save", () => {
        this.updateNationality(this.data)
            .then(res => {
               this.$notify({
                group: "foo",
                title: "Sucesso",
