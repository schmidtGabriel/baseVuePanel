...
<script>
  ...
  export default {
    ...
    computed: {
      ...mapState({
        nationalityList: ({nationality}) => nationality.list
      }),
      ...mapGetters({
    ...
    methods: {
      ...mapActions(["getNationalityList", "deleteNationality"]),
    ...
    created() {
      ...
      this.$on("deleteNationality", item => {
        ...
        buttons: [
          {
            ...
            handler: () => { 
                ...
                const index = this.nationalityList.indexOf(item, 0);
                  if (index > -1) {
